...
import org.eclipse.acceleo.module.lightingSystem.main.GenerateSA;
...
import org.xtext.example.newsamydsl.validation.MySADslValidator;

import com.google.inject.Injector;

public class ConvertCapella_SA_ToBHandler extends AbstractHandler{
					...
					project.accept(resource -> {
						if (resource instanceof IFile) {
							...
							if (file.getName().endsWith(".capella")) {
								...
								List<String> errorsList = checkCapellaSAModelSyntax3(file);
	...
	public static List<String> checkCapellaSAModelSyntax3(IFile file) {
		String filePath = file.getLocationURI().getPath();
		List<String> errors = new ArrayList<>();
		
		MySADslValidator myDSLValidator = new MySADslValidator();
		
		Injector injector = new MySADslStandaloneSetup().createInjectorAndDoEMFRegistration();
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
			...
			try {
			    String SAContent = extractXmlContent(content, 2);
				parseResult = parser.parse(new InputStreamReader(new StringInputStream(SAContent)));
				EObject objroot = parseResult.getRootASTElement();
				
				myDSLValidator.checkSASyntax((SystemAnalysis) objroot, errors);
			} catch (Exception e) {
				e.printStackTrace();
			}
