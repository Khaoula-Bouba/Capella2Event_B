 ...
 */
package org.xtext.example.newlamydsl.validation;
...
import org.xtext.example.mydsl.validation.AbstractMyLADslValidator;
 ...
 */
public class MyLADslValidator extends AbstractMyLADslValidator {
	
	@Check
	public void checkLASyntax(LogicalArchitecture architecture, List<String> errors) {
		
		
		EList<LogicalFunction> logicalFunctions = architecture.getContainedLogicalFunctionPkg().getOwnedSystemFunctions();
		EList<AbstractFunction> functions = logicalFunctions.get(0).getOwnedFunctions();
		LogicalComponentPkg logicalComponentPkg = architecture.getOwnedLogicalComponentPkg();
		
		EList<LogicalComponent> logicalComponents = logicalComponentPkg.getOwnedLogicalComponents();
		List<String> functions2 = new ArrayList<>();
		List<String> logicalComponents2 = new ArrayList<>();
		List<String> exchangeValues = getFunctionalExchanges(logicalFunctions.get(0));
		...
		//Check that the logical components don't have the same name
		for(LogicalComponent logicComponent : logicalComponents) {
			parseLogialComponents(logicComponent, exchangeValues, valuesToAddFronInit, constValuesList, errors, logicalComponents2);
		}
	...
	/** parse the logical component, get the initialization values if there are any
	 * check the uniqueness of the name value
	*/
	public void parseLogialComponents(LogicalComponent logicComponent, List<String> exchangeValues, List<String> valuesToAddFronInit,
			            List<String> constValuesList, List<String> errors, List<String> logicalComponents2) {
		
		EList<Constraint> constraintsObjs = logicComponent.getOwnedContraints();
		
        getAllInitializations(constraintsObjs, exchangeValues, valuesToAddFronInit, constValuesList, false);
		
		String entityNameValue = logicComponent.getName();
		checkNames(logicalComponents2, entityNameValue, "logical component", logicComponent, errors);
		
		List<LogicalComponent> subLogicalComponents = logicComponent.getOwnedLogicalComponents();
		for(LogicalComponent subLogComponent : subLogicalComponents) {
			parseLogialComponents(subLogComponent, exchangeValues, valuesToAddFronInit, constValuesList, errors, logicalComponents2);
		}
	}
    
    public List<String> getFunctionalExchanges(LogicalFunction logicFunction){
        ...
        functionalExchanges.addAll(logicFunction.getOwnedFunctionalExchanges());
    ...
    // check the unicity of the name value
	...
	// check if all the elelments of the constraint value belong to the list of exchanges values
